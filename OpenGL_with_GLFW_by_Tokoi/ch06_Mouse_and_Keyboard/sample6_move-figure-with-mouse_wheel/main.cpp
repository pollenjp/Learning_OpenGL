...
int main()
{
  ...
  const GLint sizeLoc(/* GLint */glGetUniformLocation(/*       GLuint   program= */program,
                                                      /* const GLchar * name   = */"size"));
  const GLint scaleLoc(/* GLint */glGetUniformLocation(/*       GLuint   program= */program,
                                                       /* const GLchar * name   = */"scale"));
  const GLint locationLoc(/* GLint */glGetUniformLocation(/*       GLuint   program= */program,
                                                          /* const GLchar * name   = */"location"));
  ...
  while (!window.shouldClose()){
    ...
    // uniform 変数に値を設定する
    // > ○○くんのために一所懸命書いたものの結局○○くんの卒業に間に合わなかったGLFW による OpenGL 入門 - p96
    // > [glUniform - OpenGL 4 Reference Pages](https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glUniform.xhtml)
    glUniform2fv(/* GLint           location= */sizeLoc,
 	               /* GLsizei         count   = */1,  // This should be 1 if the targeted uniform variable is not an array
                 /* const GLfloat * value   = */window.getSize());
    glUniform1f(/* GLint   location= */scaleLoc,            // uniform 変数の場所
                /* GLfloat v0      = */window.getScale());  // 設定する GLfloat 型の値
    glUniform2fv(/* GLint           location= */locationLoc,
 	               /* GLsizei         count   = */1,  // This should be 1 if the targeted uniform variable is not an array
                 /* const GLfloat * value   = */window.getLocation());
