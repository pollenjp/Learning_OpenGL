...
public:
         ...
         const Vertex  * vertex_array)
  {
    ...
    glGenVertexArrays(/* GLsizei n      = */ 1,                      // the number of vertex array object names to generate
                      /* GLuint  *arrays= */ &this->vertex_array_object);  // an array in which the generated vertex array object names (number) are stored
    ...
    glBindVertexArray(/* GLuint array= */ this->vertex_array_object);  // Specifies the name of the vertex array to bind.
    ...
    glGenBuffers(/* GLsizei n      = */ 1,                       // Specifies the number of buffer object names to be generated.
                 /* GLuint *buffers= */ &this->vertex_buffer_objext);  // Specifies an array in which the generated buffer object names are stored.
    ...
    glBindBuffer(/* GLenum target= */ GL_ARRAY_BUFFER,        // Purpose : Vertex attributes
                 /* GLuint buffer= */this->vertex_buffer_objext);
  ...
  {
    // 頂点配列オブジェクトを削除する
    glDeleteBuffers(/* GLsizei      n       = */ 1, /* const GLuint *buffers= */ &this->vertex_array_object);
    // 頂点バッファオブジェクトを削除する
    glDeleteBuffers(/* GLsizei      n       = */ 1, /* const GLuint *buffers= */ &this->vertex_buffer_objext);
  }
  ...
  {
    // 描画する頂点配列オブジェクトを指定する
    glBindVertexArray(/* GLuint array= */this->vertex_array_object);
  }
